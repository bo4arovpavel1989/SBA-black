...
var bkurl = require('./bkurl2.js').bkurl;
...
bkurl.resources.forEach((resource)=>{
	...
	request(options, (err, res, body)=>{
	  if(err){console.log(err);}
		...
		data.parser='alexa';
		data.rank = $('.metrics-data.align-vmiddle').eq(0).html();
		data.rank=data.rank.replace(/[^\d,]/g, '');
		data.visitTime = $('.metrics-data.align-vmiddle').eq(4).html();
